...
//		System.out.println("Burst interactions: ");
	...
	// Note - edges will be rewired until conductance is <= desiredConductance
	public ArrayList<String> rewireAddBurst(int numNodes, double startTime, double stopTime, double desiredMaxConductance) {
		final int MAXATTEMPTS = 100;
		final int MINNEIGHBORS = 3;
		final double MAXDEGREEFRACTION = 0.01;
		...
		// 2a) Make new lists with only the relevant interactions, one for internal & one for external
		// 		*** NOTE: This functionality is dependent on no two timestamps exactly matching
		System.out.println("Beginning rewiring...");
		NavigableMap<Double,DefaultWeightedEdge> window = interactions.subMap(startTime, true, stopTime, false);
		ArrayList<Double> internal = new ArrayList<Double>();
		ArrayList<Double> external = new ArrayList<Double>();
		for (Double t : window.navigableKeySet()) {
			DefaultWeightedEdge e = window.get(t);
			if (selectedVertices.contains(underlying.getEdgeSource(e))) { 
				if (selectedVertices.contains(underlying.getEdgeTarget(e))) {
					internal.add(t);
				}
				else {
					external.add(t);
		...
		System.out.println("\tInteractions identified.");
		// internal now contains all timestamps of interactions that occur entirely within the burst and
		// external now contains all timestamps of interactions that occur between a burst node and a non-burst node
		
		// 2b) Select an external interaction at random and rewire it to be internal until target conductance is reached
		//		*** NOTE: Conductance will never be increased by this process. Rewiring will be constrained to edges
		//		          that already exist in the underlying graph.
		double total = internal.size() + external.size();
		while ((external.size() / total) > desiredMaxConductance) {
			//System.out.println("\tRewiring interaction...");
			Double t = external.get(rgen.nextInt(external.size()));
			DefaultWeightedEdge oldEdge = window.get(t);
			String mine, outsider, newNeighbor;
			if (selectedVertices.contains(underlying.getEdgeSource(oldEdge))) {
				mine = underlying.getEdgeSource(oldEdge);
				outsider = underlying.getEdgeTarget(oldEdge);
			}
			else {
				mine = underlying.getEdgeTarget(oldEdge);
				outsider = underlying.getEdgeSource(oldEdge);				
			}
			//System.out.println("\t\tOld interaction: (" + mine + "," + outsider + ") at time " + t);
			
			// Make a list of neighbors of this node that are within the burst
			ArrayList<String> neighbors = (ArrayList<String>) selectedVertices.clone();
			neighbors.retainAll(Graphs.neighborListOf(underlying, mine));
			
			newNeighbor = neighbors.get(rgen.nextInt(neighbors.size()));
			DefaultWeightedEdge newEdge = underlying.getEdge(mine, newNeighbor);
			if (newEdge == null) {
				System.out.println("\nERROR: Rewire edge does NOT exist in underlying graph. Exiting...");
				System.exit(-1);
			}
			
			// Update data structures
			interactions.remove(t);
			NavigableSet<Double> timeList = interactionsByEdge.get(oldEdge);
			timeList.remove(t);
			interactions.put(t, newEdge);
			timeList = interactionsByEdge.get(newEdge);
			timeList.add(t);
			external.remove(t);
			
			//System.out.println("\t\tNew interaction: (" + mine + "," + newNeighbor + ") at time " + t);			
		}

		System.out.println("Rewiring complete.");
	...
	// Get "flat" neighborhoods of specified radius ("width") at current time as well as +/- timeDepth
	TreeSet<TemporalID> calcTemporalCylinder(String vertex, int t, int width, int timeDepth) {
		TreeSet<TemporalID> result = new TreeSet<TemporalID>();

		// Current timestep
		result.addAll(calcNowNeighborhood(vertex,t,width));
		
		int i = 1;
		while (i <= timeDepth) {
			if (t-i >= 0) {
				result.addAll(calcNowNeighborhood(vertex, t-i, width));
			}
			if (t+i < endTime) {
				result.addAll(calcNowNeighborhood(vertex, t+i, width));
			}
	...
	TreeSet<TemporalID> calcNowNeighborhood(String vertex, int t, int width) {
		TreeSet<String> nodes = new TreeSet<String>();
		TreeSet<String> newnodes = new TreeSet<String>();
		
		// Source node
		newnodes.add(vertex);
		
		while (width > 0) {
			TreeSet<DefaultWeightedEdge> extension = new TreeSet<DefaultWeightedEdge>(eComparator);
			for (String v : newnodes) {				
				extension.addAll(underlying.edgesOf(v));
			}
			nodes.addAll(newnodes);
			newnodes.clear();
			
			// Process all edges from the new nodes; if they lead to a new node that has an interaction in the desired
			// timeframe, add that node to the neighborhood
			for (DefaultWeightedEdge e : extension) {
				String s = underlying.getEdgeSource(e);
				String r = underlying.getEdgeTarget(e);
				if (!(nodes.contains(s) && nodes.contains(r))) {	// Skip "back edges"
					NavigableSet<Double> times = interactionsByEdge.get(e).subSet(t + 0.0d, true, t + 1.0d, false);
					if (times.size() > 0) {
						if (nodes.contains(s)) { newnodes.add(r); }
						else { newnodes.add(s); }
					}
				}
			}
		...
		TreeSet<TemporalID> result = new TreeSet<TemporalID>();
		for (String v : nodes) {
			TemporalID tid = new TemporalID(v,t);
			result.add(tid);
		}
