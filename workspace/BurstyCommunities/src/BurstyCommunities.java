...
public class BurstyCommunities {
	...
	private static final String graphFileName = "new_igraph4.txt";
	...
	public static void main(String args[])
	{
		...
		final int MINBINSIZE = 4;

		// for (String v1 : burst) { System.out.println(test.calcTemporalCylinder(v1,20,WIDTH,DEPTH)); }
		...
		*/
		...
		for (int i = 0; i < BANDS; i++) {
			for (ArrayList<Long> coll : (Collection<ArrayList<Long>>)sigTables[i].values()) {
				if (coll.size() >= MINBINSIZE) {
//					short max_n = 0;
//					short min_n = Short.MAX_VALUE;
					boolean in_burst = false;
					System.out.print("[");
					for (Long l : coll) { 
						System.out.print(neighborhoodSize[l.intValue()] + ":" + l + ",");
						if (((l / NV) >= startBurst) && ((l / NV) < endBurst)) {
							if (burst.contains("" + (l % NV))) {
								in_burst = true;
							}
						}
